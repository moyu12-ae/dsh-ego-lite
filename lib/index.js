...
*/
function buildAiSearchScript(args, useSpace$1, ensureRealTab$1, defaultSpace = SEARCH_SPACE) {
	const queries = (Array.isArray(args.queries) ? args.queries : []).filter((q) => typeof q === "string" && q.trim() !== "");
	const resolvedSpace = str(args.space, defaultSpace);
...
function buildPlainSearchScript(args, useSpace$1, ensureRealTab$1, defaultSpace = SEARCH_SPACE) {
	const queries = (Array.isArray(args.queries) ? args.queries : []).filter((q) => typeof q === "string" && q.trim() !== "");
	const resolvedSpace = str(args.space, defaultSpace);
	const keep = bool(args.keep, false);
...
function createSingleSpaceTracker(defaultSpace) {
	...
	try {
		...
		handle = subprocess.spawn({
			argv: useEvalArgv ? [
				...
				"nodejs",
				"-e",
				withAppFacades("app", wrapEvalBody(script))
...
*/
const spaceArg = (v, cfg, exec) => {
	return typeof v === "string" && v !== "" || typeof v === "number" ? useSpace(v) : useSpaceFallback(defaultSpaceFor(cfg, exec));
};
/** This caller's fallback space (per-agent tracker; shared when no agent). */
const defaultSpaceFor = (cfg, exec) => cfg.spaceTracker.for(exec?.agent).current();
/** Short stable suffix derived from an agent id (djb2 → base36, 6 chars). */
function agentShortId(agent) {
	const raw = agent?.id;
...
*/
const agentSearchSpace = (exec) => exec?.agent ? `${SEARCH_SPACE}@${agentShortId(exec.agent)}` : SEARCH_SPACE;
/**
...
function parseSpaceEcho(stdout, stderr) {
	for (const text of [stderr, stdout]) {
		if (!text) continue;
		const lines = text.split("\n");
		for (let i = lines.length - 1; i >= 0; i--) {
			const line = lines[i];
			if (line.startsWith(SPACE_ECHO_PREFIX)) try {
				const parsed = JSON.parse(line.slice(11));
				return {
...
function defineEgoTool(ctx, cfg, opts) {
	return defineTool({
		...
		execute: async (args, exec) => withEgoLock(async () => {
			...
			if (value && typeof value === "object" && !Array.isArray(value) && value.space === void 0) {
...
function apply(ctx, config = {}) {
	...
	const bridge = installEgoBrowserSettings(ctx, Object.fromEntries(settingKeys.filter((key) => cfgRecord[key] !== void 0).map((key) => [key, cfgRecord[key]])));
	...
	const cfg = {
		...
		spaceTracker,
		get defaultSpace() {
			return this.spaceTracker.shared.current();
		},
...
function registerActionTools(ctx, cfg, reg) {
	const t = (opts) => defineEgoTool(ctx, cfg, {
		...
		afterExecute: (args, result, exec) => {
			...
			if (opts.name === "ego_space_open") tracker.opened(args, result);
			else if (opts.name === "ego_space_close") tracker.closed(args.name, result.done);
			else if (args && args.space !== void 0 && args.space !== "") tracker.selected(args.space);
			opts.afterExecute?.(args, result, exec);
		}
	});
	const spaceParam = {
		type: "string",
		description: "Task-space name or numeric id; defaults to THIS conversation's most recently opened or explicitly selected space (per-conversation state — parallel conversations never reroute each other)."
	};
	reg(t({
		name: "ego_space_open",
		...
		buildScript: (args, exec) => `${useSpace(str(args.name, defaultSpaceFor(cfg, exec)))}console.log('${SENTINEL}' + JSON.stringify({ ok: true, id: task.id ?? null, name: task.name ?? ${j(str(args.name, defaultSpaceFor(cfg, exec)))}, note: ${j("reuse this space for follow-ups; when the goal is done run ego_space_close (keep defaults to false)")} }))\n`
	}));
	reg(t({
	...
	reg(t({
		...
		buildScript: (args, exec) => `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}await page.locator(${j(str(args.selector, ""))}).fill(${j(str(args.text, ""))})\nconst pginfo = await page.info()\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, page: pginfo }))\n`
	}));
	reg(t({
		name: "ego_js",
		...
		buildScript: (args, exec) => `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}${SAFE_FN}const result = await page.evaluate(${j(str(args.expression, ""))})\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, result: safe(result) }))\n`
	}));
	reg(t({
		name: "ego_cdp",
		...
		buildScript: (args, exec) => {
			...
			return `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}${SAFE_FN}const result = ${call}\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, result: safe(result) }))\n`;
		}
	}));
	reg(t({
		name: "ego_screenshot",
		...
		buildScript: (args, exec) => {
			...
			return `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}const path = ${shot}\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, path }))\n`;
		}
	}));
	reg(t({
		name: "ego_page_info",
	...
	reg(t({
		...
		buildScript: (args, exec) => {
			...
			return `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}await page.waitForSelector(${j(sel)}, { state: ${j(str(args.state, "visible"))}, timeout: ${num(args.timeout, 1e4)} })\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, waited: true, selector: ${j(sel)}, state: ${j(str(args.state, "visible"))} }))\n`;
		}
	}));
	reg(t({
		...
		buildScript: (args, exec) => {
			...
			return `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}const __ok = await page.waitForURL(${j(p)}, { timeout: ${num(args.timeout, 1e4)} }).catch(() => false)\nconst __u = await page.url()\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: !!__ok, reached: !!__ok, url: __u }))\n`;
		}
	}));
	reg(t({
	...
	reg(t({
		...
		buildScript: (args, exec) => {
			...
			return `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}` + (sel !== "" ? `await page.locator(${j(sel)}).hover()\n` : `await page.mouse.move(${args.x}, ${args.y})\n`) + `console.log('${SENTINEL}' + JSON.stringify({ ok: true }))\n`;
		}
	}));
	reg(t({
	...
	reg(t({
		...
		buildScript: (args, exec) => `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}await page.locator(${j(str(args.selector, ""))}).selectOption(${j(args.value ?? "")})\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, select: ${j(str(args.selector, ""))} }))\n`
	}));
	reg(t({
		name: "ego_drag",
		...
		buildScript: (args, exec) => {
			...
			return `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}` + action + `console.log('${SENTINEL}' + JSON.stringify({ ok: true }))\n`;
		}
	}));
	reg(t({
		name: "ego_scroll",
	...
	reg(t({
		...
		buildScript: (args, exec) => `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}await page.locator(${j(str(args.selector, ""))}).setInputFiles(${j(str(args.path, ""))})\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, upload: ${j(str(args.selector, ""))} }))\n`
	}));
	reg(t({
		name: "ego_download",
  ...
  const __deadline = Date.now() + Math.max(5000, ${timeout})\n  while (Date.now() < __deadline && !__path) {\n    const __now = __dlSnap()\n    if (__now !== __dlBefore) {\n      const __seen = new Set(__dlBefore ? __dlBefore.split('|') : [])\n      const __fresh = __now.split('|').find(e => !__seen.has(e))\n      if (__fresh) {\n        const __fname = __fresh.split(':')[0]\n        if (!__fname.startsWith('._') && !__fname.endsWith('.crdownload') && !__fname.endsWith('.download') && __fname !== '.DS_Store') {\n          __path = __dlDir + '/' + __fname\n          __name = __fname\n        }\n      }\n    }\n    if (!__path) await page.waitForTimeout(500)\n  }\n}\nlet __final = __path\n` + (savePath !== "" ? `if (__final && __final !== ${j(savePath)} && __dshFs) { try { __dshFs.copyFileSync(__final, ${j(savePath)}); __final = ${j(savePath)} } catch {}\n}\n` : "") + `console.log('${SENTINEL}' + JSON.stringify({ ok: true, captured: !!(__path || __name), path: __final, suggestedFilename: __name, url: __url }))\n`;
	...
	reg(t({
		...
		buildScript: (args, exec) => {
			...
			return `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}await page.locator(${j(str(args.selector, ""))}).${chk ? "check" : "uncheck"}()\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, checked: ${chk} }))\n`;
		}
	}));
	reg(t({
		name: "ego_dialog",
		...
		buildScript: (args, exec) => {
			...
			return `${spaceArg(args.space, cfg, exec)}${ensureRealTab()}const __r = await cdp("Page.handleJavaScriptDialog", ${params}).catch((e) => ({ error: String(e) }))\nconst __ok = !!(__r && !__r.error)\nconsole.log('${SENTINEL}' + JSON.stringify({ ok: true, handled: __ok, accept: ${accept}, error: __r?.error ?? null }))\n`;
		}
	}));
	reg(t({
		name: "ego_http",
	...
	reg(t({
		...
		parameters: { space: spaceParam },
		buildScript: (args, exec) => buildTabListScript(spaceArg(args.space, cfg, exec))
	}));
	reg(t({
		name: "ego_tab_switch",
	...
	reg(t({
		...
		buildScript: (args, exec) => buildScrollToBottomScript(spaceArg(args.space, cfg, exec), str(args.selector, ""), num(args.maxScrolls, 30), num(args.settleMs, 600))
	}));
	reg(t({
		name: "ego_wait_page",
		...
		buildScript: (args, exec) => {
			...
			return buildWaitPageScript(spaceArg(args.space, cfg, exec), state, num(args.timeoutMs, 15e3), num(args.idleMs, 500));
		}
	}));
	reg(t({
		...
		buildScript: (args, exec) => {
			...
			return buildDispatchKeyScript(spaceArg(args.space, cfg, exec), key, str(args.selector, ""));
		}
	}));
	reg(t({
		name: "ego_site_tool",
		...
		buildScript: (args, exec) => {
			...
			const spacePrefix = useSpace(str(args.space, "") !== "" ? args.space : defaultSpaceFor(cfg, exec));
	...
	reg(t({
		...
		parameters: {
			...
			space: {
				type: "string",
				description: `Task-space name; defaults to a per-conversation search space ('${SEARCH_SPACE}@<this-agent>') so parallel conversations never share or auto-close each other's search space. Reused across calls; auto-completed after the run unless keep=true.`
			},
			keep: {
				type: "boolean",
				description: `Keep the search space open after the run (default false). When false and the space is a tool-owned '${SEARCH_SPACE}*' space, the tool auto-completes it so it never leaks (the summary+citations are already returned, so the page is not needed). Set true to keep browsing from a citation link. A caller-passed non-default space is never auto-closed.`
			}
		},
	...
	reg(t({
		...
		parameters: {
			...
			keep: {
				type: "boolean",
				description: `Keep the search space open after the run (default false). When false and the space is a tool-owned '${SEARCH_SPACE}*' space, the tool auto-completes it so it never leaks. Set true to keep browsing from a result link. A caller-passed non-default space is never auto-closed.`
			}
		},
...
function registerHelpAndDoctor(ctx, cfg, reg) {
	reg(defineTool({
		...
		parameters: { space: {
			type: "string",
			description: "Task-space name or numeric id; defaults to THIS conversation's active space (per-conversation state)."
		} },
		output: {
			schema: {
				type: "object",
		...
		timeoutMs: 15e3,
		execute: async (args, exec) => withEgoLock(async () => {
			const result = await withWarmupRetry(() => runEgoScript(ctx.subprocess, humanCheckScript(str(args.space, defaultSpaceFor(cfg, exec))), { signal: exec?.signal }, cfg));
			if (!result.ok) throw new Error(result.error);
