...
const Config$1 = z.object({
	...
	execSession: z.union([
		...
		"persistent",
		"per-call"
	]).description("Execution channel: auto keeps one attached REPL session per app install (fast) and falls back to process-per-call spawns on anomalies."),
...
function platformManifest(platform$1 = process.platform, arch = process.arch) {
	return FFMPEG_MANIFEST[`${platform$1}-${arch}`] ?? null;
}
...
async function probeFfmpeg(path, { platform: platform$1 = process.platform, env = process.env, spawn: spawn$1 = spawn, requestedEncoder = "auto" } = {}) {
	...
	const encoders = requestedEncoder === "auto" ? platform$1 === "win32" ? [
		"h264_mf",
		"h264_nvenc",
		"h264_qsv",
		"h264_amf",
...
function getSharedFfmpegInstallationManager(options = {}) {
	...
	const key = `${platform$1}:${arch}:${cacheRoot}`;
...
var FfmpegInstallationManager = class {
	...
	checkKey;
	statusValue;
	constructor({ getConfig = () => ({}), platform: platform$1 = process.platform, arch = process.arch, env = process.env, cacheRoot = defaultFfmpegCacheRoot(), fetchImpl = globalThis.fetch, spawn: spawn$1 = spawn } = {}) {
		...
		this.spawn = spawn$1;
		this.manifest = platformManifest(platform$1, arch);
...
async function runEgoScript(subprocess, script, exec, cfg, graceOverrideMs) {
	...
	if (cfg.replCapable && !cfg.replDisabled) {
		if (cfg.execSession === "persistent" || cfg.execSession === "auto" && !cfg.replDisabled) try {
