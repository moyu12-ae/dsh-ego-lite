...
`;
const LIST_TABS_HEAD = TAB_FIND_FN + `var __tabs = await __dshPick(typeof listTabs === 'function' ? listTabs : null, typeof browser !== 'undefined' ? browser : null, 'listTabs')()\n`;
...
*/
function buildSiteToolScript(site, tool, args) {
	return SPACE_PICKER_FN + `var __run = __dshPick(typeof runSiteTool === 'function' ? runSiteTool : null, typeof site !== 'undefined' ? site : null, 'runTool')\nvar __r = await __run(${j$1(site)}, ${j$1(tool)}, ${j$1(args ?? {})})\nconsole.log('${SPACE_CONTROL_SENTINEL}' + JSON.stringify({ ok: true, site: ${j$1(site)}, tool: ${j$1(tool)}, result: __r }))\n`;
}
...
function registerActionTools(ctx, cfg, reg) {
	...
	reg(t({
		...
		buildScript: (args) => {
			...
			return buildSiteToolScript(site, tool, args.args && typeof args.args === "object" ? args.args : {});
		}
	}));
	reg(t({
		name: "web_ai_search",
